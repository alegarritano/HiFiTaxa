...
both nodes before sourcing/running, e.g. `export NXF_SINGULARITY_CACHEDIR=/scratch/$USER/hifitaxa_sif`.
...
If the `HiFiTaxa` repo (and therefore the `hifitax` package) is **private**, the
login node needs GHCR credentials to pull it. Either make the package public
(Your packages → `hifitax` → Package settings → change visibility → Public), or
...
The public `quay.io` and Docker Hub images need no authentication.
