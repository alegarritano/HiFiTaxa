...
process dada2_denoise {
    ...
    perl -i -pe 's/OMEGA_C=1e-40/OMEGA_C=$params.omegac/g' dada2_custom_script/run_dada.R
    chmod +x dada2_custom_script/run_dada.R
    export PATH="./dada2_custom_script:\$PATH"

    export PYTHONUNBUFFERED=1
    SB=""; command -v stdbuf >/dev/null 2>&1 && SB="stdbuf -oL -eL" || true   # line-buffer so progress streams to the log live
    \$SB qiime dada2 denoise-ccs --i-demultiplexed-seqs $samples_qza \
...
process dada2_denoise_with_error_model {
    ...
    cp $dada_ccs_script dada2_custom_script/run_dada.R
    perl -i -pe 's/OMEGA_C=1e-40/OMEGA_C=$params.omegac/g' dada2_custom_script/run_dada.R
    perl -i -pe 's/^/#/ if /### LEARN ERROR RATES ###/../### PROCESS ALL SAMPLES ###/' dada2_custom_script/run_dada.R
    perl -i -pe 's/#### LEARN ERROR RATES ###/#### LEARN ERROR RATES ###\\nerr <- readRDS("$error_model")/' dada2_custom_script/run_dada.R
    chmod +x dada2_custom_script/run_dada.R
    export PATH="./dada2_custom_script:\$PATH"
